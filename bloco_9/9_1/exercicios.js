...
const messageDelay = () => Math.floor(Math.random() * 8000);
...
// sendMarsTemperature(); // imprime "It is currently 47ºF at Mars", por exemplo
...
const messageDelay = () => Math.floor(Math.random() * 5000);
const taxSucess = () => Math.floor(Math.random() * 100);
...
// definição da função sendMarsTemperature...
;
const sendMarsTemperature = (callback, callbackError)  => (taxSucess() <= 60) ? callback(getMarsTemperature()) : callbackError('Robot is busy');
